...
class OptimizedInfiniteGameOfLifeSuite extends FunSuite {

  test("should create a game of life") {
    val gameOfLife = GameOfLife.createOptimized().makeAlive((0,0))
    assert(gameOfLife.cellAt((0, 0)) == alive)
    assert(gameOfLife.cellAt((1, 1)) == dead)
  }

  test("should parse graphical representation") {
   val gameOfLife: GameOfLife = GameOfLife.parseOptimized(
      """x_
        |__""".stripMargin)
